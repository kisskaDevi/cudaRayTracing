...
namespace bloom {
    ...
    __device__ vec4 bloomColor(ray r, size_t maxIterations, hitableList* list, curandState* local_rand_state) {
        ...
        if ((i < width) && (j < height)) {
            ...
            for (size_t sampleIndex = 0; sampleIndex < samplesCount; sampleIndex++) {
                ...
                for (size_t rayIndex = 0; rayIndex < raysCount; rayIndex++) {
                    frameBuffer[pixel_index] += bloomColor(camRay, hitCount, list, &local_rand_state);
                }
            }
        }
    }
